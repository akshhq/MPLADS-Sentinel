...
const FALLBACK_NATIONAL_ANALYTICS = {
  ...
  riskCounts: { critical: 34, high: 127, medium: 842, low: 17429 },
  ...
  async getNationalAnalytics() {
    if (isConfigured && supabase) {
      try {
        ...
        if (!error && data) {
          return {
            totalWorksMonitored: data.total_works_monitored,
            totalSanctionedCr: data.total_sanctioned_cr,
            totalDisbursedCr: data.total_disbursed_cr,
            totalFlaggedRiskValueCr: data.total_flagged_risk_value_cr,
            riskCounts: data.risk_counts || { critical: 34, high: 127, medium: 842, low: 17429 },
            riskTrend7D: data.risk_trend_7d || [],
            riskTrend30D: data.risk_trend_30d || [],
