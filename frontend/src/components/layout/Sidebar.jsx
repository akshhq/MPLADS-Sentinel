...
import { LayoutDashboard, FolderKanban, ShieldAlert, FileCheck, SearchCode, BarChart3, Sparkles, Database, ShieldCheck, ExternalLink, ScanLine, } from "lucide-react";
...
export const Sidebar = ({ onCloseMobile }) => {
    ...
    return (<aside className="w-64 h-screen flex flex-col bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 select-none">
      {/* Brand Header */}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        {navItems.map((item) => {
            ...
            return (<Link key={item.href} href={item.href} onClick={onCloseMobile} className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition-all ${isActive
              ...
              {item.badge && (<span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${isActive ? "bg-white/20 text-white" : item.badgeColor || "bg-slate-100 text-slate-600"}`}>
                  {item.badge}
                </span>)}
            </Link>);
        })}
      ...
      <div className="p-3 border-t border-slate-100 dark:border-slate-800 space-y-2">
        <Link href="/" target="_blank" className="flex items-center justify-between px-3 py-2 rounded-xl text-xs text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
