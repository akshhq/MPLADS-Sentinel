      ...
      <header className="sticky top-0 z-30 h-16 w-full glass-panel border-b border-slate-200/80 dark:border-slate-800/80 px-4 md:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-xs text-slate-400 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200/70 dark:hover:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700/80 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-blue-700 bg-blue-50 dark:bg-blue-950/60 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/80 rounded-xl border border-blue-200 dark:border-blue-800 transition-colors shadow-sm"
          >
          ...
          <div className="relative" ref={menuRef}>
            ...
            {/* User Dropdown Menu */}
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl py-2 z-50 text-xs animate-in fade-in zoom-in-95 duration-100">
                ...
                <div className="p-2">
                  ...
                  {Object.entries(DEMO_PERSONAS).map(([key, p]) => (
                    <button
                      key={key}
                      onClick={() => {
                        switchPersona(key as SentinelRole);
                        setIsUserMenuOpen(false);
                      }}
                ...
                <div className="p-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
                  <Link
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-lg flex items-center gap-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                  >
