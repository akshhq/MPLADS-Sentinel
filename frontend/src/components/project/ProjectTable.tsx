...
import { ArrowRight, MapPin, Building, ArrowUpDown, ChevronLeft, ChevronRight, Download } from "lucide-react";
...
export const ProjectTable: React.FC<ProjectTableProps> = ({ projects, total }) => {
  const [sortField, setSortField] = useState<keyof Project | "sanctioned" | "risk">("risk");
  const [sortAsc, setSortAsc] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 10;

  const handleSort = (field: keyof Project | "sanctioned" | "risk") => {
    if (sortField === field) {
      setSortAsc(!sortAsc);
    } else {
      setSortField(field);
  ...
  const exportCSV = () => {
    ...
    const rows = projects
      .map(
        (p) =>
          `"${p.id}","${p.title.replace(/"/g, '""')}","${p.category}","${p.state}","${p.district}",${p.financials.sanctionedAmount},${p.financialProgress},${p.physicalProgress},${p.risk.score},"${p.risk.level}"`
      )
      .join("\n");
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
        ...
        <button
          onClick={exportCSV}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors font-semibold"
        >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden flex">
                      <div
                        ...
                        title={`Financial Progress: ${project.financialProgress}%`}
                      />
                      <div
                        className="bg-emerald-500 h-full"
                        style={{ width: `${project.physicalProgress}%` }}
                        title={`Physical Progress: ${project.physicalProgress}%`}
                      />
                    </div>
                    {Math.abs(project.financialProgress - project.physicalProgress) >= 25 && (
