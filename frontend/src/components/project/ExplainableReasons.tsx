...
import { ShieldAlert, AlertTriangle, ArrowRight, FileCheck, CheckCircle2, Cpu } from "lucide-react";
import { RiskReason } from "@/types/risk";
...
export const ExplainableReasons: React.FC<ExplainableReasonsProps> = ({
  reasons,
  className = "",
}) => {
  return (
    <div className={`p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4 ${className}`}>
      ...
      <div className="space-y-3">
        {reasons.map((reason, idx) => (
          <div
            key={reason.id || idx}
            className="p-4 rounded-xl bg-slate-50/80 dark:bg-slate-850/60 border border-slate-200/80 dark:border-slate-800 space-y-3"
          >
            {/* Title & Score Pill */}
            ...
            {reason.deviations && reason.deviations.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-1">
                {reason.deviations.map((dev, dIdx) => (
                  <div
                    key={dIdx}
                    className="flex items-center gap-2 px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-[11px] font-mono"
                  >
            ...
            {reason.recommendedAction && (
              <div className="p-2.5 rounded-lg bg-blue-50/60 dark:bg-blue-950/30 border border-blue-200/60 dark:border-blue-900/40 text-[11px] flex items-start gap-2">
