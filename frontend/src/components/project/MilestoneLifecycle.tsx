...
interface MilestoneLifecycleProps {
  milestones: Milestone[];
  className?: string;
}

export const MilestoneLifecycle: React.FC<MilestoneLifecycleProps> = ({
  milestones,
  className = "",
}) => {
  return (
        ...
        <div className="min-w-[650px] grid grid-cols-5 gap-3">
          {milestones.map((m, idx) => {
            ...
            return (
              <div
                key={m.id}
                className={`p-3.5 rounded-xl border ${statusStyles.border} space-y-2 flex flex-col justify-between`}
              >
                ...
                <div className="space-y-1.5 pt-2 border-t border-current/10 text-[11px]">
                  ...
                  <span
                    className={`inline-block w-full text-center py-0.5 rounded text-[10px] font-bold ${statusStyles.pill}`}
                  >
                    {statusStyles.label}
                  </span>
                </div>
