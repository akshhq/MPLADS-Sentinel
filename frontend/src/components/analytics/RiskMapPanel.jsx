...
export const RiskMapPanel = ({ points, className = "" }) => {
    ...
    return (<div className={`p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 relative aspect-[4/3] rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center p-4">
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            return (<button key={pt.id} onClick={() => setSelectedPoint(pt)} style={{ left: coords.x, top: coords.y }} className={`absolute -translate-x-1/2 -translate-y-1/2 p-1.5 rounded-full ${markerColor} transition-all duration-200 ${isSelected
                    ? "ring-4 scale-125 z-20 shadow-lg shadow-rose-900/50"
                    : "ring-2 hover:scale-110 z-10 opacity-90"}`} title={`${pt.projectTitle} (${pt.district})`}>
        ...
        <div className="lg:col-span-4 flex flex-col justify-between p-5 rounded-2xl bg-slate-50 dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800 space-y-4">
          {selectedPoint ? (<div className="space-y-4">
              ...
              <Link href={`/app/projects/${selectedPoint.projectId}`} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 shadow-sm transition-all">
                <span>Inspect Digital Project Twin</span>
                <ArrowRight className="w-3.5 h-3.5"/>
              </Link>
            </div>) : (<p className="text-xs text-slate-400 italic">Select a marker on the map to inspect.</p>)}
        </div>
      </div>
    </div>);
};
