...
import { UploadCloud, FileText, CheckCircle2, AlertTriangle, Sparkles, X, ArrowRight } from "lucide-react";

interface FileUploadDropzoneProps {
  onFileSelect: (file: File | null, customText?: string, sampleName?: string) => void;
  isAnalyzing?: boolean;
}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleClear}
                className="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-1"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {sampleDocuments.map((s, idx) => (
              <button
                ...
                className="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 hover:border-purple-400 dark:hover:border-purple-600 text-left transition-all group shadow-2xs flex flex-col justify-between"
              >
