...
export const RiskBadge = ({ level, score, showIcon = true, size = "md", className = "", }) => {
    ...
    return (<span className={`inline-flex items-center rounded-full border transition-all ${styles.bg} ${styles.text} ${styles.border} ${sizeClasses} ${className}`}>
      {showIcon && <span className="shrink-0">{getIcon()}</span>}
      <span>{styles.label}</span>
      {score !== undefined && (<span className="opacity-80 font-mono tracking-tight text-[0.9em] border-l border-current/20 pl-1.5">
          {score}/100
        </span>)}
    </span>);
};
