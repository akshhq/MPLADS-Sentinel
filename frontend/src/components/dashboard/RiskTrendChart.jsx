...
export const RiskTrendChart = ({ data }) => {
    ...
    const displayData = timeRange === "7D"
        ? safeData.slice(-3)
        : timeRange === "30D"
            ? safeData.slice(-4)
            : timeRange === "3M"
    ...
    return (<div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col justify-between h-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
        ...
        <div className="flex items-center p-1 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 self-start sm:self-auto">
          {["7D", "30D", "3M", "1Y"].map((range) => (<button key={range} onClick={() => setTimeRange(range)} className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition-all ${timeRange === range
      ...
      <div className="h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={displayData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#94a3b8" }} tickLine={false} axisLine={{ stroke: "#e2e8f0", className: "dark:stroke-slate-800" }}/>
            <YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} tickLine={false} axisLine={false}/>
            <Tooltip content={({ active, payload, label }) => {
            if (active && payload && payload.length) {
                return (<div className="p-3 rounded-xl bg-slate-900 text-white text-xs shadow-xl border border-slate-800 space-y-1">
            ...
            <Area type="monotone" dataKey="flaggedAnomalies" stroke="#f43f5e" strokeWidth={2.5} fillOpacity={1} fill="url(#flaggedGradient)" name="Flagged Anomalies"/>
            <Area type="monotone" dataKey="avgRiskScore" stroke="#3b82f6" strokeWidth={2} fillOpacity={1} fill="url(#scoreGradient)" name="Avg Risk Score"/>
          </AreaChart>
        </ResponsiveContainer>
      </div>
