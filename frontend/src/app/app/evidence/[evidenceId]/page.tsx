...
export default function EvidenceDetailPage({ params }: PageProps) {
  ...
  if (loading) {
    return (
      <AppShell breadcrumbs={[{ label: "Evidence", href: "/app/evidence" }, { label: evidenceId }]}>
  ...
  return (
    <AppShell breadcrumbs={[{ label: "Evidence", href: "/app/evidence" }, { label: evidence.id }]}>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Linked to Project:{" "}
              <Link
                href={`/app/projects/${evidence.projectId}`}
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link
              href={`/app/projects/${evidence.projectId}`}
              className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 text-white hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 transition-all shadow-sm"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3">
              ...
              {evidence.type === "image" ? (
                <div className="aspect-video rounded-xl bg-slate-900 border border-slate-700 flex flex-col items-center justify-center p-6 text-center text-white relative overflow-hidden">
                  ...
                  {evidence.metadata?.gpsLatitude && (
                    <div className="absolute bottom-3 left-3 right-3 flex justify-between text-[11px] font-mono bg-black/60 px-3 py-1.5 rounded-lg">
                ...
                <div className="p-6 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-4 font-mono text-xs">
                  ...
                  {evidence.extractedFields && evidence.extractedFields.length > 0 ? (
                    <div className="space-y-2">
                      {evidence.extractedFields.map((field, idx) => (
                        <div
                          key={idx}
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            {field.mismatchNote && (
                              <p className="text-[10px] text-rose-600 dark:text-rose-400 mt-1 max-w-xs font-sans">
                                {field.mismatchNote}
                              </p>
                            )}
                          </div>
                    ...
                    <p className="text-slate-400 text-center py-4">Standard Treasury Ledger Record</p>
                  )}
                </div>
              )}
            </div>
              ...
              {evidence.findings && evidence.findings.length > 0 ? (
                <div className="space-y-3">
                  {evidence.findings.map((f) => (
                    <div
                      key={f.id}
                      className="p-4 rounded-xl bg-rose-50/70 dark:bg-rose-950/30 border border-rose-200/80 dark:border-rose-900/40 space-y-2 text-xs"
                    >
                ...
                <div className="p-4 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200/80 dark:border-emerald-900/40 text-xs text-emerald-800 dark:text-emerald-300 space-y-1">
