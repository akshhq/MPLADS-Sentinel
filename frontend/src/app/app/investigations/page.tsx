...
export default function InvestigationsQueuePage() {
  ...
  return (
    <AppShell breadcrumbs={[{ label: "Investigations" }]}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 shadow-sm self-start md:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            title="Active Cases"
            value="8"
            subtitle="Prioritized for audit review"
            trend={{ value: "3 Urgent Priority", isPositive: true, isPositiveGood: false }}
            icon={SearchCode}
            variant="default"
          />
          <MetricCard
            ...
            value="4"
            subtitle="Assigned to nodal officers"
            trend={{ value: "Field inspections active", isPositive: true, isPositiveGood: true }}
            icon={Clock}
            variant="warning"
          />
          <MetricCard
            ...
            value="2"
            subtitle="Awaiting photo / ledger proof"
            trend={{ value: "Varanasi & Bengaluru", isPositive: false, isPositiveGood: false }}
            icon={AlertTriangle}
            variant="warning"
          />
          <MetricCard
            ...
            value="14"
            subtitle="Verified without irregularity"
            trend={{ value: "Audited cases", isPositive: true, isPositiveGood: true }}
            icon={CheckCircle2}
            variant="default"
          />
        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5 whitespace-nowrap">
                          <RiskBadge
                            level={c.riskScore >= 80 ? "critical" : c.riskScore >= 60 ? "high" : "medium"}
                            score={c.riskScore}
                            size="sm"
                          />
                        </td>
                        ...
                        <td className="px-4 py-3.5 whitespace-nowrap">
                          <span
                            className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${statusStyles.bg} ${statusStyles.text} ${statusStyles.border}`}
                          >
                            {statusStyles.label}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3.5 text-right whitespace-nowrap">
                          <Link
                            href={`/app/investigations/${c.id}`}
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-900 hover:text-white dark:hover:bg-blue-600 transition-all shadow-xs"
                          >
                            <span>Workspace</span>
                            <ArrowRight className="w-3.5 h-3.5" />
                          </Link>
