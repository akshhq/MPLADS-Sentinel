...
import { getCaseStatusStyles, formatDateTime, formatIndianCurrency } from "@/lib/formatters";
...
export default function InvestigationWorkspacePage({ params }: PageProps) {
  ...
  const handleUpdateStatus = async (status: CaseStatus, note?: string) => {
    if (!caseItem) return;
    try {
  ...
  if (loading) {
    return (
      <AppShell breadcrumbs={[{ label: "Investigations", href: "/app/investigations" }, { label: caseId }]}>
  ...
  return (
    <AppShell
      breadcrumbs={[{ label: "Investigations", href: "/app/investigations" }, { label: caseItem.id }]}
      contextProjectId={caseItem.projectId}
      ...
      <div className="space-y-6">
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2 flex-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${statusStyles.bg} ${statusStyles.text} ${statusStyles.border}`}
              >
                {statusStyles.label}
              </span>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
              <span>
                Target Project:{" "}
                <Link
                  href={`/app/projects/${caseItem.projectId}`}
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0 pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-100 dark:border-slate-800">
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 transition-colors shadow-xs"
            >
              <Printer className="w-3.5 h-3.5" />
              <span>Export Audit Brief</span>
            </button>

            {caseItem.status !== "evidence_requested" && (
              <button
                onClick={() =>
                  handleUpdateStatus(
                    "evidence_requested",
                    "Formal requisition issued to District Nodal Officer requesting fresh geotagged site imagery and ledger reconciliation."
                  )
                }
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-purple-50 text-purple-700 dark:bg-purple-950 dark:text-purple-300 hover:bg-purple-100 border border-purple-200 dark:border-purple-800 transition-colors"
              >
            ...
            {caseItem.status !== "escalated" && (
              <button
                onClick={() =>
                  handleUpdateStatus(
                    "escalated",
                    "Case escalated to Central Performance Audit Directorate for formal administrative inquiry."
                  )
                }
            ...
            {caseItem.status !== "cleared" && (
              <button
                onClick={() =>
                  handleUpdateStatus(
                    "cleared",
                    "Case cleared following comprehensive field verification and satisfactory engineering inspection."
                  )
                }
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 hover:bg-emerald-100 border border-emerald-200 dark:border-emerald-800 transition-colors"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <EvidenceChain
              chain={
                caseItem.evidenceChain && caseItem.evidenceChain.length > 0
              ...
              <div className="space-y-2">
                {evidenceList.map((e) => (
                  <Link
                    ...
                    className="p-3 rounded-xl bg-slate-50 dark:bg-slate-850 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 flex items-center justify-between transition-colors group"
                  >
