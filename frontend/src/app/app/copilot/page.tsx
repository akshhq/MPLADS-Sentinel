...
export default function CopilotPage() {
  const [messages, setMessages] = useState<CopilotMessage[]>([
    {
      ...
      content:
        "Welcome to the Sentinel AI Copilot Workspace. I am directly grounded in 18,432 MPLADS project records, PFMS treasury transactions, OCR document extractions, and official MoSPI 2023 Scheme Guidelines. How can I assist your audit investigation today?",
    },
  ]);
  ...
  return (
    <AppShell breadcrumbs={[{ label: "AI Copilot" }]}>
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            onClick={handleClear}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors self-start sm:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {suggestedQuestions.map((sq, idx) => (
            <button
              ...
              className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 hover:border-blue-400 dark:hover:border-blue-600 text-left text-xs font-medium text-slate-700 dark:text-slate-300 transition-all flex items-center justify-between group shadow-xs"
            >
              <span>{sq}</span>
        ...
        <div className="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col min-h-[500px] overflow-hidden">
          ...
          <div className="flex-1 p-6 space-y-6 overflow-y-auto">
            {messages.map((msg) => {
              ...
              return (
                ...
                >
                  {!isUser && (
                    <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-blue-700 to-blue-500 text-white flex items-center justify-center shrink-0 mt-1 shadow-sm">
                      <Bot className="w-4 h-4" />
                    </div>
                  )}

                  <div
                    className={`max-w-[85%] rounded-2xl p-5 text-xs space-y-4 ${
                      isUser
                      ...
                      <div className="pt-3 border-t border-slate-200 dark:border-slate-700 space-y-2">
                        ...
                        <div className="space-y-1.5">
                          {msg.structuredResponse.riskSignals.map((rs, idx) => (
                            <div
                              key={idx}
                              className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-rose-200/60 dark:border-rose-900/40 space-y-0.5"
                            >
                      ...
                      <div className="pt-3 border-t border-slate-200 dark:border-slate-700 space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {msg.structuredResponse.evidenceSources.map((src, idx) => (
                            <span
                              key={idx}
                              className="px-2.5 py-1 rounded-lg text-[11px] font-mono font-bold bg-blue-50 text-blue-800 dark:bg-blue-950/80 dark:text-blue-300 border border-blue-200 dark:border-blue-800"
                            >
                              {src.title}
                            </span>
                          ))}
                    ...
                    {msg.structuredResponse?.guidelinesCited && (
                      <div className="pt-3 border-t border-slate-200 dark:border-slate-700 space-y-2">
                        ...
                        {msg.structuredResponse.guidelinesCited.map((g, idx) => (
                          <div
                            key={idx}
                            className="p-3 rounded-xl bg-purple-50/50 dark:bg-purple-950/30 border border-purple-200/60 dark:border-purple-900/40 text-[11px]"
                          >
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50">
            ...
            >
              <input
                type="text"
                placeholder="Ask about project risks, evidence provenance, or statutory guidelines..."
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                className="flex-1 px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-xs"
              />
              <button
                type="submit"
