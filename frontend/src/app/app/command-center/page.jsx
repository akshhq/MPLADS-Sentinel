...
import { ShieldAlert, ShieldCheck, Building2, BadgeIndianRupee, Calendar, Sparkles, Camera, CopyCheck, FileText, Clock, MapPin, Landmark, HardHat, SearchCode, ClipboardCheck, } from "lucide-react";
...
export default function CommandCenterPage() {
    ...
    return (<AppShell breadcrumbs={[{ label: "Command Center" }]}>
      <div className="space-y-6">
        {/* Role Jurisdiction Greeting Banner */}
        {profile && (<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <select value={selectedRegion} onChange={(e) => setSelectedRegion(e.target.value)} aria-label="Select State / UT Region" className="px-3 py-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 shadow-xs focus:outline-none">
            ...
            <Link href="/app/risk/documents/compare" className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 shadow-xs transition-all">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard title="Works Monitored" value="18,432" subtitle="100% of sanctioned works screened" trend={{ value: "+14.2% YoY", isPositive: true, isPositiveGood: true }} icon={Building2} variant="default"/>
          <MetricCard title="High Risk Cases" value="127" subtitle="Prioritized for nodal inquiry" trend={{ value: "+8 this month", isPositive: true, isPositiveGood: false }} icon={ShieldAlert} variant="warning"/>
          <MetricCard title="Critical Risk Cases" value="34" subtitle="Multi-source inconsistency verified" trend={{ value: "Urgent Review", isPositive: false, isPositiveGood: false }} icon={ShieldAlert} variant="critical"/>
          <MetricCard title="Flagged Sanction Value" value="₹42.8 Cr" subtitle="Across prioritized anomalous works" trend={{ value: "0.87% of total funds", isPositive: false, isPositiveGood: true }} icon={BadgeIndianRupee} variant="default"/>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8">
            {analytics ? (<RiskTrendChart data={analytics.monthlyTrends}/>) : (<div className="h-80 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-shimmer"/>)}
          </div>
          <div className="lg:col-span-4">
            {analytics ? (<RiskDonutChart distribution={analytics.riskDistribution}/>) : (<div className="h-80 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-shimmer"/>)}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Link href="/app/risk" className="text-xs font-semibold text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white hover:underline">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            const Icon = mod.icon;
            return (<Link key={mod.label} href={mod.href} className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600 shadow-xs hover:shadow-sm transition-all group">
