...
import { ArrowLeft, MapPin, Building, ShieldAlert, ArrowRight, TrendingUp } from "lucide-react";
import { AppShell } from "@/components/layout/AppShell";
import { MetricCard } from "@/components/common/MetricCard";
import { LoadingSkeleton } from "@/components/common/LoadingSkeleton";
import { api } from "@/lib/api";
import { StateMetric, DistrictMetric } from "@/types/analytics";

interface PageProps {
  params: Promise<{ state: string }>;
}

export default function StateAnalyticsPage({ params }: PageProps) {
  const resolvedParams = use(params);
  const stateSlug = resolvedParams.state;

  const [stateData, setStateData] = useState<StateMetric | null>(null);
  ...
  if (loading) {
    return (
      <AppShell breadcrumbs={[{ label: "Analytics", href: "/app/analytics" }, { label: stateSlug }]}>
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Link
            href="/app/analytics"
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
          >
        ...
        {stateData && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard
              ...
              value={stateData.totalWorks.toLocaleString("en-IN")}
              subtitle="Total state portfolio"
              trend={{ value: "Active surveillance", isPositive: true, isPositiveGood: true }}
              icon={Building}
              variant="default"
            />
            <MetricCard
              title="Total Sanctioned"
              value={`₹${stateData.totalSanctionedCr} Cr`}
              subtitle={`₹${stateData.totalExpenditureCr} Cr disbursed`}
              trend={{ value: "PFMS connected", isPositive: true, isPositiveGood: true }}
              icon={TrendingUp}
              variant="default"
            />
            <MetricCard
              title="High / Critical Works"
              value={`${stateData.highRiskWorks + stateData.criticalWorks}`}
              subtitle={`${stateData.criticalWorks} critical priority`}
              trend={{ value: "Prioritized audits", isPositive: true, isPositiveGood: false }}
              icon={ShieldAlert}
              variant="critical"
            />
            <MetricCard
              title="State Risk Index"
              value={`${stateData.averageRiskScore} / 100`}
              subtitle={stateData.primaryRiskFactor.split(" ")[0]}
              trend={{ value: "State baseline", isPositive: true, isPositiveGood: true }}
              icon={Building}
              variant="default"
            />
          </div>
                ...
                {districts.map((d) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-right font-sans whitespace-nowrap">
                      <Link
                        href={`/app/projects?state=${encodeURIComponent(d.state)}&district=${encodeURIComponent(d.district)}`}
