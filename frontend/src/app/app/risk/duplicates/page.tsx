...
import { CopyCheck, MapPin, ArrowRight, ShieldAlert, Cpu, Sparkles, Building, CheckCircle2, Split } from "lucide-react";
...
export default function DuplicateIntelligencePage() {
  const [activeTab, setActiveTab] = useState<"cluster1" | "custom">("cluster1");

  return (
    <AppShell breadcrumbs={[{ label: "Risk Intelligence", href: "/app/risk" }, { label: "Duplicate Intelligence" }]}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold text-white bg-slate-900 dark:bg-blue-600 hover:bg-slate-800 dark:hover:bg-blue-700 shadow-sm self-start md:self-auto"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            ...
            value="19"
            subtitle="Potential scope overlaps"
            trend={{ value: "SBERT > 85%", isPositive: true, isPositiveGood: false }}
            icon={CopyCheck}
            variant="default"
          />
          <MetricCard
            title="Overlap Sanctioned"
            value="₹14.8 Cr"
            subtitle="Co-located proposals"
            trend={{ value: "Flagged for survey", isPositive: false, isPositiveGood: false }}
            icon={ShieldAlert}
            variant="warning"
          />
          <MetricCard
            title="Geospatial Proximity (<500m)"
            value="12 Pairs"
            subtitle="Adjacent village works"
            trend={{ value: "Cadastral check", isPositive: true, isPositiveGood: false }}
            icon={MapPin}
            variant="critical"
          />
          <MetricCard
            title="Deduplication Model"
            value="SBERT + GIS"
            subtitle="Cosine + Haversine Radius"
            trend={{ value: "v2.0 Active", isPositive: true, isPositiveGood: true }}
            icon={Cpu}
            variant="default"
          />
        </div>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-4">
            ...
            <div className="flex items-center gap-2">
              <Link
                ...
                className="px-3.5 py-1.5 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
            ...
            <div className="p-5 rounded-2xl bg-slate-50/80 dark:bg-slate-850/60 border border-slate-200/80 dark:border-slate-800 space-y-4">
              ...
              <Link
                href="/app/projects/MPL-004821"
                className="w-full flex items-center justify-center gap-1 py-2 rounded-xl text-xs font-bold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 hover:text-blue-600 transition-colors"
              >
