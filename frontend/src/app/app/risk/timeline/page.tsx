...
import { Clock, AlertTriangle, ShieldAlert, ArrowRight, TrendingDown, CheckCircle2, Calendar } from "lucide-react";
...
export default function TimelineRiskPage() {
  ...
  return (
    <AppShell breadcrumbs={[{ label: "Risk Intelligence", href: "/app/risk" }, { label: "Timeline Risk" }]}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard
            title="Delayed Works"
            value="38"
            subtitle="Past scheduled milestone"
            trend={{ value: "+5 this month", isPositive: true, isPositiveGood: false }}
            icon={Clock}
            variant="warning"
          />
          <MetricCard
            title="Average Delay"
            value="41 Days"
            subtitle="Across flagged projects"
            trend={{ value: "Max 64 days (Varanasi)", isPositive: false, isPositiveGood: false }}
            icon={TrendingDown}
            variant="default"
          />
          <MetricCard
            title="High Deadline Risk (>80%)"
            value="14"
            subtitle="Predicted delivery failure"
            trend={{ value: "Urgent escalation", isPositive: true, isPositiveGood: false }}
            icon={ShieldAlert}
            variant="critical"
          />
          <MetricCard
            title="On-Track Completion Rate"
            value="91.4%"
            subtitle="National compliance baseline"
            trend={{ value: "+2.1% YoY", isPositive: true, isPositiveGood: true }}
            icon={CheckCircle2}
            variant="default"
          />
        </div>
                        ...
                        <div className="w-24 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${
                              item.deadlineRiskPercent >= 80 ? "bg-rose-600" : "bg-amber-500"
                            }`}
                            style={{ width: `${item.deadlineRiskPercent}%` }}
                          />
                        </div>
                      </div>
                    </td>
