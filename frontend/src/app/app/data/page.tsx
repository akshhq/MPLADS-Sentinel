...
import { Database, Search, Download, Table, CheckCircle2, FileSpreadsheet, ArrowRight } from "lucide-react";
...
export default function DataExplorerPage() {
  const [datasets, setDatasets] = useState<GroundDataset[]>([]);
  const [selectedDatasetId, setSelectedDatasetId] = useState<string>("DS-REC-01");
  ...
  return (
    <AppShell breadcrumbs={[{ label: "Data Explorer" }]}>
                      ...
                      {currentDataset.columns.map((col) => (
                        <td key={col.key} className="px-4 py-3 whitespace-nowrap">
                          {col.dataType === "currency" && typeof row[col.key] === "number"
                            ? formatIndianCurrency(row[col.key] as number)
                            : String(row[col.key])}
                        </td>
                      ))}
                    </tr>
