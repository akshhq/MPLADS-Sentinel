...
export default function LoginPage() {
  ...
  const handleDemoLogin = (targetRole) => {
    ...
    setSuccessMsg(`Authenticated as ${DEMO_PERSONAS[targetRole].full_name} (${DEMO_PERSONAS[targetRole].designation})`);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 py-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Left Side: Info & 7 Official Quick Demo Personas */}
        <div className="lg:col-span-7 space-y-6">
          <div className="space-y-3">
            ...
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">
              Institutional MPLADS Governance Access
            </h1>
            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
              Access is governed by <strong>Role + Jurisdiction + Assignment + Permission</strong>. Select any of the 7 official stakeholder accounts below to access specialized command centers, tailored inspection queues, and evidence chains.
            </p>
          </div>
              ...
              {roleOrder.map((key) => {
                ...
                return (
                  <button
                    ...
                    className="text-left p-3 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-500 bg-slate-50/70 dark:bg-slate-800/60 hover:bg-blue-50/50 dark:hover:bg-blue-950/40 transition-all group active:scale-[0.98]"
                  >
        ...
        {/* Right Side: Auth Form Card (Sign In Only) */}
          ...
          <div className="bg-white dark:bg-slate-900 p-6 sm:p-7 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-5">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Sign in with authorized institutional credentials
              </p>
            </div>

            {/* Admin Policy Notice */}
            <div className="p-3 rounded-xl bg-blue-50/80 dark:bg-blue-950/40 border border-blue-200/80 dark:border-blue-800/80 text-[11px] text-blue-700 dark:text-blue-300 flex items-start gap-2">
              <Info className="w-4 h-4 shrink-0 mt-0.5" />
              <span>
                <strong>Institutional Access Only</strong>: Public self-registration is disabled. User accounts and jurisdictions are provisioned exclusively by the Platform System Administrator.
              </span>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="ministry@mpladssentinel.demo"
                    className="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••••••"
                    className="w-full pl-10 pr-10 py-2 text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-700 shadow-md transition-all flex items-center justify-center gap-2 active:scale-[0.99] disabled:opacity-60"
              >
                <span>{loading ? "Authenticating..." : "Sign In to Sentinel"}</span>
            ...
            <div className="pt-2 text-center border-t border-slate-100 dark:border-slate-800">
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                Storage: <span className="font-mono text-blue-600 dark:text-blue-400">Supabase Cloud</span> • Auth: <span className="font-mono text-emerald-600 dark:text-emerald-400">RBAC Token Layer</span>
              </p>
            </div>
          </div>
        </div>
