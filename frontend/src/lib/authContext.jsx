...
export const AuthProvider = ({ children }) => {
    ...
    const [profile, setProfile] = useState(DEMO_PERSONAS.mospi_officer); // Default MoSPI Officer
    ...
    const fetchUserProfile = async (authUser) => {
        ...
        try {
            const { data, error } = await supabase
                ...
                .single();
            if (!error && data) {
                const role = data.role || "mospi_officer";
                ...
                const role = meta.role || "mospi_officer";
    ...
    const signInWithPassword = async (email, password) => {
        if (!isSupabaseConfigured || !supabase) {
            const persona = Object.values(DEMO_PERSONAS).find((p) => p.email.toLowerCase() === email.toLowerCase()) ||
